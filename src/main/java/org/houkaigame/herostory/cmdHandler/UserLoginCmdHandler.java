...
import org.houkaigame.herostory.login.LoginService;
...
public class UserLoginCmdHandler implements  ICmdHandler<GameMsgProtocol.UserLoginCmd> {
    ...
    public void handle(ChannelHandlerContext ctx, GameMsgProtocol.UserLoginCmd cmd) {
        ...
        //如果正在处理登录操作
        if (USER_LOGIN_STATE_MAP.containsKey(userName))return;
    ...
    /**
     * 清理超时登录时间
     * userLoginTimeMap 用户登陆时间字典
     */
    private void clearTimeoutLoginTime(Map<String, Long> userLoginTimeMap) {
        if (null==userLoginTimeMap ||userLoginTimeMap.isEmpty())return;
